 ...
 *  시설물 목록을 얻는다
 * 
 * @author 신인철(zlola@naver.com)
 */
public class GetFacility implements Handler {
    ...
    public void perform(Map<String, String> kvp, HttpServletResponse res)
        ...
        ArrayList<ConditionSearch> cslist = CS.getFacilityAll();  //선택한 시군구 전체 목록을 들고옴
        ...
        for (int i = 0; i < cslist.size(); i++) {//불러온DB테이블에서 하나씩 증가하며 원하는 값을 찾는다
            ...
            sb.append("<시설물테이블명>").append(cslist.get(i).getValue()).append("</시설물테이블명>");
            sb.append("</시설물>");
