...
REDIPS.drag = (function () {
	...
	var	init,						// initialization
		...
		hover_color = '#000000',	// (string) hover color
	...
	set_color = function () {
		...
		if (table < tables.length && table !== null && row !== null && cell !== null) {
			...
			tables[table].rows[row].cells[cell].style.border = REDIPS.drag.hover_color;
	...
	calculate_cells = function () {
		...
		// open loop for each HTML table inside id=drag (table array is initialized in () function)
	...
	save_content = function (tbl) {
		...
		var tblsearch = document.getElementById("table1");
		var tbl_rows = tblsearch.rows.length;
		var tbl_cells = celllength;
		

		for(var i=0; i<tbl_cells; i++){
			for(var j=0; j<tbl_rows; j++){
				if(tblsearch.rows[j].cells[i]){
					tbl_cell = tblsearch.rows[j].cells[i];
					if (tbl_cell.childNodes.length > 0){
						for (var d = 0; d < tbl_cell.childNodes.length; d++){
							if (tbl_cell.childNodes[d].tagName == 'DIV'){
								...
								query += tbl_cell.childNodes[d].id + ',';
							}
						}
					}
				}				
		...
		/*for (t = tbl_start; t <= tbl_end; t++) {  //���̺�
			tbl_rows = tables[t].rows.length;
			for (r = 0; r < tbl_rows; r++) { //tr
	...
	return {
		...
		myhandler_notmoved		: function () {
			...
			var urlpath = $("#"+obj.id).find("#url_"+obj.id).text();
			...
			//window.open(urlpath, "pop", "top=0, left=0, fullscreen, status=no, resizable=yes, location=no, toolbar=no, menubar=no");*/
			
		},
		myhandler_dropped		: function () {
			fnb.connect.workupdate(save_content());
		}, 
