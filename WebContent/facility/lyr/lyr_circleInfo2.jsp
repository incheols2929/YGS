...
<%
	...
	ArrayList<ConditionSearch> relist = CS.getSrchLryCircle(tbl_id, colkey, cx, cy,  distance, srch_cond, psnum, ps, lyr_nm); //결과목록을 가져온다.
	...
	if(TL.size() != 0){
		...
		String excelAuth = Code.getAuthCheck("FW010", id, session_ugrp_id);
...
<div id="lyr_wapp" class style="border:0px solid #000000; height:100%;">
	<div id="lyr_sunject" class="border3" style="height:25px;">
		<button onclick="_PC.init.frameClose();" style="position: absolute; right: 5px; width: 55px; cursor: pointer;">닫기</button>
	...
	<div id="lyr_text" style="height:25px;">
		...
		<div style="float: right; margin:2px 3px 0 0;">
			<img src="img/excel_btn.gif" style="cursor: pointer;" onclick="<%=excelChk%>"/>	
		</div>
	</div>
		...
		<table border="0" cellpadding="0" cellspacing="0" height="auto" width="100%" class="border1">
			...
			<tr>
				<td align="center" colspan="<%=colspan%>" height="30" bgcolor="#eeeeee">
					...
					%>
						<img src='./img/prev.gif' border='0' align='absmiddle'  style='cursor: pointer;' onclick="parent.index._CS.init.getLryCircleInfopage('<%=lyr_nm%>', '<%=lyr_id%>', '<%=srch_cond%>', '<%=resultcnt%>', '<%=cx%>', '<%=cy%>', '<%=distance%>', '<%=po.startPage - po.pageBlock%>');"/>
					<%		
						}
					%>
					<%
						for (int i = po.startPage; i <= po.endPage; i++) {
							if(i == po.currentPage){
					%>
								<a href="#" onclick="parent.index._CS.init.getLryCircleInfopage('<%=lyr_nm%>', '<%=lyr_id%>', '<%=srch_cond%>', '<%=resultcnt%>', '<%=cx%>', '<%=cy%>', '<%=distance%>', '<%=i%>');"><b><%=i %></b></a>
					<%
							}else{
					%>
								<a href="#" onclick="parent.index._CS.init.getLryCircleInfopage('<%=lyr_nm%>', '<%=lyr_id%>', '<%=srch_cond%>', '<%=resultcnt%>', '<%=cx%>', '<%=cy%>', '<%=distance%>', '<%=i%>');"><%=i %></a>
					<%
							}
							if(i != po.endPage){
					%>
					...
					%>
						<img src='./img/next.gif' border='0' align='absmiddle' style='cursor: pointer;' onclick="parent.index._CS.init.getLryCircleInfopage('<%=lyr_nm%>', '<%=lyr_id%>', '<%=srch_cond%>', '<%=resultcnt%>', '<%=cx%>', '<%=cy%>', '<%=distance%>', '<%=po.startPage + po.pageBlock%>');"/>
	...
	<div  id="excelfrm" style="display:none; position: absolute; top:50%; left:50%; margin:-100px 0 0 -160px; width:350px; height:250px; background-color:#ffffff;">
		<table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%" class="table_border">
			...
			<tr>
				<td height="25" colspan="3" align="center" bgcolor="#e3e4e4"><a href="#" onclick="_PC.init.excelLogInsertCrir();">확인</a> | <a href="#" onclick="_PC.init.excelFormClose();">닫기</a></td>
			</tr>
			...
			<input type="hidden" id="cy" name="cy" value="<%=cy%>"/>
