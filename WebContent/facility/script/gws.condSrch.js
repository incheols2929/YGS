...
_CS.init = {
		...
		//새주소에서 시설물검색
		srchFacilityLoding : function(){
			$.ajax({
				url: "./search/srchFacility.jsp",///전송페이지
				...
				success: function(htm){//전송 성공하면 실행문
					...
					_CS.init.GetSGG2();
					/*_CS.init.locationInfo();*/
					//_CS.init.GetFacility();
					$("#sch_line").html("");
		...
		//저수지 저수율 읍면 주소 을 가져온다.
		locationInfo : function(){
	    	  $.ajax({
			       type: "POST",
					url:"./search/cntxml/locationInfo_juso.jsp", 
					dataType : "xml",
					success: function(xml){
			    	   var conditions_box = "";
			    	   conditions_box+= $("#locationInfo").find("option").remove().end().append("<option value=''></option>");
			           $(xml).find('gbn').each(function(idx){
			           ...
			           $("#locationInfo").append(conditions_box);
			...
			GetSGG2 : function(sggcode){
			    ...
			    $("#facility").removeOption(/./);
			    $("#facility").addOption("", ":: 선택하여주십시오! ::", true);
			...
			GetUMD2 : function(sggcode){
		       ...
		       $("#ri").addOption("", ":: 선택하여주십시오! ::", true);
		       $("#facility").removeOption(/./);
		       $("#facility").addOption("", ":: 선택하여주십시오! ::", true); //초기화시 ""공란에 값추가
		       
		       if(sggcode != ""){
				   $.ajax({
				       type: "POST",
				       url: "../../svc",
				       data: {svc : "GetUMD2", sgg : sggcode},
				       dataType: "xml",
				       success: function(xml) {
				    	   var conditions_box = "";
		...
		GetFacility : function(){
			    ...
			    $("#facility").addOption("", ":: 선택하여주십시오! ::", true); //초기화시 ""공란에 값추가
				   $.ajax({
						type: 'POST',
				    	url: '../../svc',
				    	data : {svc : "GetFacility"},
				    	dataType: 'xml',
				    	  success: function(xml) {
					    	   var conditions_box = "";
					           $(xml).find('시설물').each(function(idx){
					               var code = $(this).find('시설물코드').text();
					               var name = $(this).find('시설물명').text();
					               
					               var table = $(this).find('시설물테이블명').text();
					               
					               conditions_box += "<option value='" + code + "'>" + name + "</option>";
					              
						         //  $("#facility").addOption(code, name, false);
					           });
					           $("#facility").append(conditions_box);
					       },
				    	error: function(){ 
				        	alert('Error loading XML document'); 
				    	}
		...
		GetUmd3 : function(){
			var sggcode = "46870";
			   $("#umd").removeOption(/./);
		       $("#umd").addOption("", ":: 읍면동 선택 ::", true);
		   	   $("#res").removeOption(/./);
		       $("#res").addOption("", ":: 저수지 선택 ::", true);
		...
		//저수지 명을 가져온다.
		GetReservoir_info : function(umdcode){
			$("#res").removeOption(/./);
		    $("#res").addOption("", ":: 저수지 선택 ::", true);
		    if(umdcode != ""){
		    	$.ajax({
				       type: "POST",
				       url: "../../svc",
				       data: {svc : "GetReservoir", umd : umdcode},
				       dataType: "xml",
				       success: function(xml) {
				    	   var conditions_box = "";
				           $(xml).find('시설물').each(function(idx){
		...
		//리 목록을가져온다.
		/*GetRi3 : function(umdcode){
			alert(umdcode);
	       $("#ri").removeOption(/./);
	       $("#ri").addOption("", ":: 리 선택 ::", true);
	       if(umdcode != ""){
			   $.ajax({
			       type: "POST",
			       url: "../../svc",
      ...
      //시설물 코드를 찾음
		facilitySrch:function(){
			var facility = $("#facility").val();
			
			$.ajax({ //외부에 데이터를 전송하거나 요청
				type:"GET",
				url:"./search/cntxml/facility_srch.jsp",
				data : {facility:facility}, //서버로 전송할 데이터
				dataType : "xml",
				success: function(xml){ //성공헀을 때
						$(xml).find('data').each(function(){
							var code = $(this).find('facility').text();
							
							_CS.init.GetResultContentfacility(code);
        ...
        facilityReset:function(){
			...
			_CS.init.Facilitypaging(0);
        },
		
        Facilitypaging : function(pagenum){
			
        	var sgg = $("#sgg").val(); //입력된 이름을 변수 sgg에 담음			
			var umd = $("#umd").val();  // 위와 동일
			var ri = $("#ri").val(); 
			var facility = $("#facility").val(); //입력된 이름을 변수 sgg에 담음
			var rsv_nm = $("#rsv_nm").val();

			/*
		    if(rsv_nm == null || rsv_nm ==""){
			...
			$.ajax({ //외부에 데이터를 전송하거나 요청
				type:"GET",
				url:"./search/cntxml/facility_cnt.jsp",
				data : {sgg:sgg, umd:umd, ri:ri, facility:facility, rsv_nm:rsv_nm}, //서버로 전송할 데이터
				dataType : "xml",
				success: function(xml){ //성공헀을 때
					if($(xml).find("data").length != 0){
						...
						_CS.init.GetResultContentfacility(sgg, umd, ri, facility, rsv_nm, _PA.info.start, _PA.info.pageSize); //지번검색
		...
		*/
		...
		GetResultContentfacility : function(code){
			...
			*/
			if(facility == ""){ 
				...
				$.ajax({
				       type: "POST",
				       url: "../../svc", 
				       data: {svc : "GetFaciliAddrSrchList", sgg:sgg, umd:umd, ri:ri, facility:facility,ftrnm:ftrnm},
				       dataType: "xml",
				       contentType:"application/x-www-form-urlencoded; charset=UTF-8", //한글로 검색되는 값을 가져오기 위해
				       success: function(xml){
				    	   var str="";
				    	   if($(xml).find('시설물').length != 0){
				    		   $(xml).find('시설물').each(function(idx, item){
				    		   var ftrcde = $(this).find("시설구분").text();
				    		   var ri_cde = $(this).find("리코드").text();
					    	   ...
					    	   str+="<div style='margin-top:3px;'>";
								str+="<table border='0' cellpadding='0' cellspacing='0' width='100%;' height='auto' bgcolor='#ffffff' style='border:1px solid #86c0ef'>";
									str+="<tr align='center'>";
					    				...
					    				str+="<td width='65%' align='left' style='border-bottom:1px dotted #c9c8c8;border-right:1px dotted #c9c8c8;'>&nbsp;<a href='#' onclick='_MAINMAP.searchAndMoveNoScale(\""+lyr_nm +"\", \"" + gid+ "\");'>" + juso + "</a></td>";
										str+="<td width='10%' style='border-bottom:1px dotted #c9c8c8;'>";
												/*str+="<img src='img/info_.gif' onclick='_MAINMAP.searchAndMoveNoScale(\""+ lyr_nm+"\", \"" + gid+ "\");' style='cursor:pointer;'/>";*/
												str+="<img src='img/info_.gif' onclick='_MAINMAP.searchAndMove(\""+ lyr_nm+"\", \"" + gid+ "\",\"500\");' style='cursor:pointer;'/>";
											str+="</td>";
										str+="</tr>";
										    str+="<tr align='center'>";
											str+="<td height='20' bgcolor='#b2dcfd' style='border-bottom:1px dotted #c9c8c8; border-right:1px dotted #c9c8c8;'>시설명</td>";
											str+="<td align='left' style='border-bottom:1px dotted #c9c8c8; border-right:1px dotted #c9c8c8;'>&nbsp;"+ ftr_nm +"</td>";
											str+="<td style='border-bottom:1px dotted #c9c8c8;'>";	
											if(ftr_nm != ""){
												if(ftrcde == "FTR001"){ //저수지
													str+="<img src='img/info.gif'onclick='_MAINMAP.setReservoirPop(\""+ftr_idn+"\");' style='cursor:pointer;'/>";
								    			}else if(ftrcde == "FTR002"){//배수장
								    				str+="<img src='img/info.gif'onclick='_MAINMAP.setDrainPop(\""+ftr_idn+"\");' style='cursor:pointer;'/>";
								    			}else if(ftrcde == "FTR003"){//양수장
								    				str+="<img src='img/info.gif'onclick='_MAINMAP.setPumpPop(\""+ftr_idn+"\");' style='cursor:pointer;'/>";
								    			}else if(ftrcde == "FTR004"){//관정
								    				str+="<img src='img/info.gif'onclick='_MAINMAP.setTbwPop(\""+ftr_idn+"\");' style='cursor:pointer;'/>";
								    			}else if(ftrcde == "FTR005"){//집수암거
								    				str+="<img src='img/info.gif'onclick='_MAINMAP.setCulvertPop(\""+ftr_idn+"\");' style='cursor:pointer;'/>";
								    			}else if(ftrcde == "FTR008"){//방조제
								    				str+="<img src='img/info.gif'onclick='_MAINMAP.setSeawallPop(\""+ftr_idn+"\");' style='cursor:pointer;'/>";
								    			}else if(ftrcde == "FTR009"){//취입보
								    				str+="<img src='img/info.gif'onclick='_MAINMAP.setCwipPop(\""+ftr_idn+"\");' style='cursor:pointer;'/>";
								    			}else if(ftrcde == "FTR010"){//저류지
								    				str+="<img src='img/info.gif'onclick='_MAINMAP.setBasinPop(\""+ftr_idn+"\");' style='cursor:pointer;'/>";
								    			}
											}else{
					           ...
					           $("#result_content").html(str);
				    		   	...
				    		   	var pstr = "";
								pstr +="<div style='border:2px dotted red; text-align:center; width:70%; background-color:#ffffff; position: absolute; height:50px; top:50%; left:50%; margin:-30px 0 0 -105px;'>";
	...
	/*GetResultContentfacility : function(code){
		...
		if(facility == ""){ 
			...
			$.ajax({
			       type: "POST",
			       url: "../../svc",
			       data: {svc : "GetFaciliAddrSrchList", sgg:sgg, umd:umd, ri:ri, facility:facility,rsvnm:rsvnm},
			       dataType: "xml",
			       contentType:"application/x-www-form-urlencoded; charset=UTF-8", //한글로 검색되는 값을 가져오기 위해
			       success: function(xml){
			    	   var str="";
			    	   if($(xml).find('srch-list').length != 0){
			    		   $(xml).find('시설물').each(function(idx, item){
			    			
			    			var ftridn = $(this).find("관리번호").text();
			    			var mngcde = $(this).find("관리기관").text();
			    			var pnu = $(this).find("PNU").text();
			    			var jibunaddr = $(this).find("주소").text();
			    			var facilinm = $(this).find("시설명").text();
			    			var mngnam = $(this).find("관리자").text();
			    			var telnum = $(this).find("관리자연락처").text();
			    			var begymd = $(this).find("착공일자").text();
			    			var fnsymd = $(this).find("준공일자").text();
			    			var rsvvol = $(this).find("저수량").text();
			    			var pdgunt = $(this).find("단위저수량").text();
			    			var fulara = $(this).find("만수면적").text();
			    			var formexp = $(this).find("형식").text();
			    			var byclen = $(this).find("제당연장").text();
			    			var totjea = $(this).find("제고최고").text();
			    			var avgjea = $(this).find("제고평고").text();
			    			var byclen = $(this).find("정폭").text();
			    			var latinsug = $(this).find("법면내제").text();
			    			var latoutsu = $(this).find("법면외제").text();
			    			var sloinsug = $(this).find("보호공내제").text();
			    			var slooutsu = $(this).find("보호공외제").text();
			    			var rsvara = $(this).find("유역면적").text();
			    			var drawal = $(this).find("홍수량").text();
			    			var orfdth = $(this).find("일류수심").text();
			    			var sctcmd = $(this).find("구조형식").text();
			    			var wswlen = $(this).find("언체연장").text();
			    			var maxundra = $(this).find("언체고최고").text();
			    			var avgundra = $(this).find("언체고평균").text();
			    			var rendra = $(this).find("방수로연장").text();
			    			var maxdra = $(this).find("방수로폭최대").text();
			    			var mindra = $(this).find("방수로폭최소").text();
			    			var avgdra = $(this).find("방수로폭평균").text();
			    			var wtrdra = $(this).find("홍수위상제고").text();
			    			var cdewtl = $(this).find("통관형식").text();
			    			var renwtl = $(this).find("통관연장").text();
			    			var sechwtl= $(this).find("통관단면h").text();
			    			var secbwtl= $(this).find("통관단면b").text();
			    			var secpwtl = $(this).find("통관단면p").text();
			    			var strwtl= $(this).find("취수시설구조").text();
			    			var secwtl= $(this).find("취수공개소").text();
			    			var stdwtl= $(this).find("취수공규격").text();
			    			var intwtl= $(this).find("취수능력").text();
			    			var maiway= $(this).find("용수간선").text();
			    			var braway= $(this).find("용수지선").text();
			    			var chuway = $(this).find("추도").text();
			    			var racway = $(this).find("도수로").text();
			    			var aquway= $(this).find("송수로").text();
			    			var drainway= $(this).find("배수로").text();
			    			var strara = $(this).find("인가").text();
			    			var visara = $(this).find("내한면적").text();
			    			var triara = $(this).find("부족면적").text();
			    			var oprnam = $(this).find("시공자").text();
			    			var gcnnam = $(this).find("도급자").text();
			    			var natamt = $(this).find("국비보조").text();
			    			var bndamt = $(this).find("융자").text();
			    			var citamt = $(this).find("시군도비").text();
			    			var selamt = $(this).find("자부담").text();
			    			var celamt = $(this).find("기타").text();
			    			var tctamt = $(this).find("합계").text();
						 ...
						 str+="<div style='margin-top:3px;'>";
							str+="<table border='0' cellpadding='0' cellspacing='0' width='100%;' height='auto' bgcolor='#ffffff' style='border:1px solid #86c0ef'>";
								str+="<tr align='center'>";
								    ...
								    str+="<tr align='center'>";
									str+="<td height='20' bgcolor='#b2dcfd' style='border-bottom:1px dotted #c9c8c8; border-right:1px dotted #c9c8c8;'>시설명</td>";
									str+="<td align='left' style='border-bottom:1px dotted #c9c8c8; border-right:1px dotted #c9c8c8;'>&nbsp;"+ facilinm +"</td>";
									str+="<td style='border-bottom:1px dotted #c9c8c8;'>";	
									
									if(facilinm != ""){
										str+="<img src='img/info.gif'onclick='_MAINMAP.setFacilityPop(\""+pnu+"\",\""+ facilinm+"\");' style='cursor:pointer;'/>"; //_DE.doroInfo(\"" + pnu + "\", \"\"); 무인도서 상세정보
									}else{
										str+="<img src='img/info_off_.gif'/>";
