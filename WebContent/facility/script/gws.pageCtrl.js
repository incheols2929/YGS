...
_PC.init = {
	...
	Reservoir_pop : function(){
		...
		popId = window.open("./form/reservoir_page.jsp", '저수지이력관리', winState);
		popId.focus();
	},
	//저수지 이력관리 추가창
	Reservoir_insert_pop : function(){
		var width = 500;
		var height = 500;
		var x = (screen.availWidth- width)/2;
		var y = (screen.availHeight- height)/2;
		var winState = 'top=' + y + ',left=' + x + ',width=' +width +',height=' +height; winState +=',menubar=no,scrollbars=yes,status=no,resizable=no';
		popId = window.open("/facility/form/Reservoir_insert_pop.jsp", '저수지이력관리추가', winState);
		popId.focus();
	},
	//개별주택가격상세팝업
