...
function getMapImg(){
	...
	if(use_resn == ""){  //사용목적이 아무것도 적혀있지 않다면
		alert("사용목적을 입력 하여주십시오!"); //이 메시지가 뜸
		$("#use_resn").focus();
		return false;
	}
	...
	$.ajax({
		...
		success: function(xml){
			if($(xml).find("result").length != 0){//xml에서 result태그를 찾아서 길이가 0과 같이 않으면
				var retult = $(xml).find("result").text();//result에 새로운 text를 받고
				if(retult == "success"){ //result가 success이면 getMapImage메소드 실행하고 close
					//부모창의 이미지저장 스크립트 호출
					opener.parent.index._MAINMAP.getMapImage();
					window.close();
				}else{
...
<table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%" class="table_border">
	<tr>
		<td colspan="3" height="25" align="center" class="border-line2" bgcolor="#c4d7f1"><b>- 이미지 사용목적 입력창 -</b></td>
	</tr>
	<tr align="center">
	...
	<input type="hidden" id="func_id" name="func_id" value="FW002"/><!-- 단위기능관리ID -->
